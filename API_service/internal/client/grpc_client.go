...
func NewGrpcClient(cfg configs.Config) *GrpcClient {
	conn, err := grpc.Dial(cfg.Session.GrpcAddress, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Printf("[ERROR] [API-Service] Failed to connect to GRPC-Session Client: %v", err)
		return nil
	}
	client := pb.NewSessionServiceClient(conn)
	log.Println("[INFO] [API-Service] Successful connect to GRPC-Session Client")
