...
migrate-up:
	...
	migrate -path ./UserManagement_service/schema -database "postgres://postgres:my_password@localhost:5434/users?sslmode=disable" up
	@echo "Applying Photo-Service migrations..."
	migrate -path ./Photo_service/schema -database "postgres://postgres:my_password@localhost:5433/photos?sslmode=disable" up
migrate-down:
	@echo "Rolling back Photo-Service migrations..."
	migrate -path ./Photo_service/schema -database "postgres://postgres:my_password@localhost:5433/photos?sslmode=disable" down
	@echo "Rolling back User-Service migrations..."
	migrate -path ./UserManagement_service/schema -database "postgres://postgres:my_password@localhost:5434/users?sslmode=disable" down
dockerstop:
...
start:
	...
	cmd /C start powershell -NoExit -Command "cd $(Logs_Service_DIR); .\logs_service.exe"
	@echo "Logs Service started."
	@echo "Building and starting API-Service..."
	cd $(API_Service_DIR) && go build -o api_service.exe cmd/main.go
	cmd /C start powershell -NoExit -Command "cd $(API_Service_DIR); .\api_service.exe"
	...
	cmd /C start powershell -NoExit -Command "cd $(Session_Service_DIR); .\session_service.exe"
	...
	cmd /C start powershell -NoExit -Command "cd $(User_Service_DIR); .\user_service.exe"
	...
	cmd /C start powershell -NoExit -Command "cd $(Photo_Service_DIR); .\photo_service.exe"
