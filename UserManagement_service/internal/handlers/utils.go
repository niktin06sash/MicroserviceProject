...
func checkMethod(r *http.Request, w http.ResponseWriter, expectedMethod string, traceID string, mapa map[string]string, place string, kafkaproducer kafka.KafkaProducerService) bool {
	if r.Method != expectedMethod {
		...
		response.SendResponse(r.Context(), w, false, nil, mapa, http.StatusBadRequest, traceID, place)
		return false
	}
	return true
}
...
func getUserIdAndSession(r *http.Request, w http.ResponseWriter, traceID string, mapa map[string]string, place string, kafkaproducer kafka.KafkaProducerService) (bool, string, string) {
	...
	if !ok {
		...
		response.SendResponse(r.Context(), w, false, nil, mapa, http.StatusInternalServerError, traceID, place)
		return false, "", ""
	}
...
func serviceResponse(resp *service.ServiceResponse, r *http.Request, w http.ResponseWriter, traceID string, place string) bool {
	if !resp.Success {
		stringMap := response.ConvertErrorsToString(resp.Errors)
		switch resp.Type {
		case erro.ClientErrorType:
			response.SendResponse(r.Context(), w, false, nil, stringMap, http.StatusBadRequest, traceID, place)
		case erro.ServerErrorType:
			response.SendResponse(r.Context(), w, false, nil, stringMap, http.StatusInternalServerError, traceID, place)
		}
		return false
	}
	return true
