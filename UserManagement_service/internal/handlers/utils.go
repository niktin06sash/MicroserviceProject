...
func (h *Handler) checkMethod(r *http.Request, w http.ResponseWriter, expectedMethod string, traceID string, place string) bool {
	if r.Method != expectedMethod {
		...
		response.BadResponse(r, w, http.StatusBadRequest, erro.ClientError(erro.ErrorInvalidReqMethod), traceID, place, h.LogProducer)
...
func getAllData[T any](r *http.Request, w http.ResponseWriter, traceID string, place string, parser T, logproducer LogProducer) bool {
	...
	if err != nil {
		...
		response.BadResponse(r, w, http.StatusBadRequest, erro.ClientError(erro.ErrorInvalidDataReq), traceID, place, logproducer)
		metrics.UserErrorsTotal.WithLabelValues(erro.ClientErrorType).Inc()
		return false
	}
...
func (h *Handler) getQueryParameters(r *http.Request, w http.ResponseWriter, traceID string, place string, personmapa map[string]string) bool {
	...
	case Update:
		...
		if (updateName != "" && updatePassword != "") || (updateName != "" && updateEmail != "") || (updatePassword != "" && updateEmail != "") {
			...
			response.BadResponse(r, w, http.StatusBadRequest, erro.ClientError(erro.ErrorInvalidCountQueryParameters), traceID, place, h.LogProducer)
			metrics.UserErrorsTotal.WithLabelValues(erro.ClientErrorType).Inc()
			return false
		}
		...
		default:
			...
			response.BadResponse(r, w, http.StatusBadRequest, erro.ClientError(erro.ErrorInvalidQueryParameter), traceID, place, h.LogProducer)
			metrics.UserErrorsTotal.WithLabelValues(erro.ClientErrorType).Inc()
			return false
		}
...
func (h *Handler) getDinamicParameters(r *http.Request, w http.ResponseWriter, traceID string, place string, personmapa map[string]string) bool {
	...
	case GetUserById:
		...
		if !ok {
			...
			response.BadResponse(r, w, http.StatusBadRequest, erro.ClientError(erro.ErrorInvalidDinamicParameter), traceID, place, h.LogProducer)
			metrics.UserErrorsTotal.WithLabelValues(erro.ClientErrorType).Inc()
			return false
		}
