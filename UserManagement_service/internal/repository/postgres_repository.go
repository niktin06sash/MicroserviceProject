...
func (repoap *AuthPostgresRepo) CreateUser(ctx context.Context, tx *sql.Tx, user *model.Person) *DBRepositoryResponse {
	...
	if err != nil {
		log.Printf("[ERROR] [UserManagement] [TraceID: %s] CreateUser: %v", traceid, err)
	...
	log.Printf("[INFO] [UserManagement] [TraceID: %s] CreateUser: Successful create person!", traceid)
...
func (repoap *AuthPostgresRepo) GetUser(ctx context.Context, useremail, userpassword string) *DBRepositoryResponse {
	...
	log.Printf("[INFO] [UserManagement] [TraceID: %s] GetUser: Successful get person!", traceid)
