...
func (s *SessionService) ValidateSession(ctx context.Context, sessionid string, flag string) *ServiceResponse {
	...
	bdresponse, serviceresponse := s.requestToDB(s.Repo.GetSession(ctx, sessionid, flag), traceID)
	if serviceresponse != nil {
		return serviceresponse
	}
	if flag == "false" {
...
func (s *SessionService) requestToDB(response *repository.RepositoryResponse, traceid string) (*repository.RepositoryResponse, *ServiceResponse) {
