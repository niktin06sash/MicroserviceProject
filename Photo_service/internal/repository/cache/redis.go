...
func NewRedisConnection(cfg configs.RedisConfig) (*CacheObject, error) {
	...
	if err != nil {
		redisobject.Close()
		log.Printf("[DEBUG] [Photo-Service] Failed to establish Redis-Client connection: %v", err)
		return nil, err
	}
	log.Println("[DEBUG] [Photo-Service] Successful connect to Redis-Client")
...
func (r *CacheObject) Close() {
	r.connect.Close()
	log.Println("[DEBUG] [Photo-Service] Successful close Redis-Client")
}
